...
const getChatsAndNotify = async (req, res) => {
    try {
      ...
      // Fetch messages received by the logged-in user that are unread
      ...
      for (const doc of receivedMessagesSnapshot.docs) {
        ...
        const notificationId = userNotificationsRef.id;
        ...
        notifications.push({
          ...
          const userNotificationsRef = db.collection("users").doc(loggedInUserId).collection("notifications");
          await userNotificationsRef.add(notification); // Store each notification
      ...
      // Count the number of notifications for the logged-in user
      const userNotificationsRef = db.collection("users").doc(loggedInUserId).collection("notifications");
      const notificationCountSnapshot = await userNotificationsRef.where("seen", "==", false).get();
